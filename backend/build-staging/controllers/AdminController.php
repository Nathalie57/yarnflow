...
class AdminController
{
     ...
     * @modified 2025-12-19 - Ajout projets et crédits photos
     */
    public function getUserDetails(int $userId): void
    {
        ...
        // [AI:Claude] Projets de l'utilisateur (v0.15.0)
        $stmtProjects = $db->prepare("
            SELECT id, name, technique, type, status, is_favorite, created_at, updated_at,
                   (SELECT COUNT(*) FROM project_rows WHERE project_id = projects.id) as total_rows
            FROM projects
            WHERE user_id = ?
        ...
        // [AI:Claude] Crédits photos (v0.15.0)
        // Vérifier quelles colonnes existent réellement
        $columnsCheck = $db->query("SHOW COLUMNS FROM user_photo_credits")->fetchAll(\PDO::FETCH_COLUMN);

        $creditsColumns = ['user_id'];
        if (in_array('monthly_credits', $columnsCheck)) $creditsColumns[] = 'monthly_credits';
        if (in_array('purchased_credits', $columnsCheck)) $creditsColumns[] = 'purchased_credits';
        if (in_array('credits_used_this_month', $columnsCheck)) $creditsColumns[] = 'credits_used_this_month';
        if (in_array('total_credits_used', $columnsCheck)) $creditsColumns[] = 'total_credits_used';
        if (in_array('last_purchase_at', $columnsCheck)) $creditsColumns[] = 'last_purchase_at';
        if (in_array('last_monthly_reset_at', $columnsCheck)) $creditsColumns[] = 'last_monthly_reset_at';

        $stmtCredits = $db->prepare("
            SELECT " . implode(', ', $creditsColumns) . "
            FROM user_photo_credits
            WHERE user_id = ?
        ");
        ...
        $projectStats = [
            ...
            'favorites' => count(array_filter($projects, fn($p) => $p['is_favorite'] == 1))
        ];
        ...
        Response::success([
            ...
            'stats' => [
                ...
                'projects_in_progress' => $projectStats['in_progress'],
                'projects_completed' => $projectStats['completed']
            ]
        ]);
    }
     ...
     * @modified 2025-12-19 - Support PLUS/PRO (v0.15.0)
     */
    public function updateUserSubscription(int $userId): void
    {
        ...
        $validator
            ...
            ->in($data['subscription_type'] ?? null, [
                SUBSCRIPTION_FREE,
                SUBSCRIPTION_PLUS,
        ...
        // [AI:Claude] Calculer la date d'expiration selon le plan
        if ($subscriptionType === SUBSCRIPTION_PLUS)
            $expiresAt = date('Y-m-d H:i:s', strtotime('+1 month'));
        elseif ($subscriptionType === SUBSCRIPTION_PLUS_ANNUAL)
            $expiresAt = date('Y-m-d H:i:s', strtotime('+1 year'));
        elseif ($subscriptionType === SUBSCRIPTION_PRO)
            $expiresAt = date('Y-m-d H:i:s', strtotime('+1 month'));
        elseif ($subscriptionType === SUBSCRIPTION_PRO_ANNUAL)
            $expiresAt = date('Y-m-d H:i:s', strtotime('+1 year'));
    ...
    /**
     * [AI:Claude] Gérer les crédits photos d'un utilisateur (ajout/retrait manuel)
    ...
    public function manageUserCredits(int $userId): void
    {
        ...
        $validator = new Validator();
        $validator
            ->required($data['credits'] ?? null, 'credits')
            ->isInteger($data['credits'] ?? null, 'credits')
            ->required($data['action'] ?? null, 'action')
            ->in($data['action'] ?? null, ['add', 'remove'], 'action');
        ...
        if ($credits <= 0)
            Response::error('Le nombre de crédits doit être positif', HTTP_UNPROCESSABLE);

        try {
            $creditManager = new \App\Services\CreditManager();

            if ($action === 'add') {
                $creditManager->addPurchasedCredits($userId, $credits);
                $message = "{$credits} crédits ajoutés";
            } else {
                // [AI:Claude] Retirer des crédits (purchased d'abord, puis monthly si nécessaire)
                $db = $this->userModel->getDb();
                $stmt = $db->prepare("
                    UPDATE user_photo_credits
                    SET purchased_credits = GREATEST(0, purchased_credits - ?)
                    WHERE user_id = ?
                ");
                $stmt->execute([$credits, $userId]);
                $message = "{$credits} crédits retirés";
            }

            // [AI:Claude] Logger l'action
            error_log("[ADMIN CREDITS] {$userData['email']} a {$action} {$credits} crédits pour user {$userId}");

            // [AI:Claude] Retourner le nouvel état
            $stmtCredits = $db->prepare("SELECT * FROM user_photo_credits WHERE user_id = ?");
            $stmtCredits->execute([$userId]);
            $updatedCredits = $stmtCredits->fetch(\PDO::FETCH_ASSOC);
            ...
            error_log('[AdminController] Erreur gestion crédits : '.$e->getMessage());
            Response::serverError('Erreur lors de la gestion des crédits');
        }
    }

    /**
     * [AI:Claude] Mettre à jour le rôle d'un utilisateur
    ...
    public function updateUserRole(int $userId): void
    {
        ...
        $validator
            ...
            ->in($data['role'] ?? null, ['user', 'admin'], 'role');
        ...
        try {
            ...
            // [AI:Claude] Logger l'action
            error_log("[ADMIN ROLE] {$userData['email']} a changé le rôle de user {$userId} ({$user['email']}) en {$role}");

            Response::success([
                'role' => $role
            ], HTTP_OK, "Rôle mis à jour en {$role}");

        } catch (\Exception $e) {
            error_log('[AdminController] Erreur mise à jour rôle : '.$e->getMessage());
            Response::serverError('Erreur lors de la mise à jour du rôle');
        }
    }

    /**
     * [AI:Claude] Bannir/débannir un utilisateur
     * PUT /api/admin/users/{id}/ban
     * Body: { "is_banned": true } ou { "is_banned": false }
     * @created 2025-12-19
     */
    public function toggleBan(int $userId): void
    {
        ...
        $data = json_decode(file_get_contents('php://input'), true);
        $isBanned = isset($data['is_banned']) ? (bool)$data['is_banned'] : null;

        if ($isBanned === null)
            Response::error('Paramètre is_banned requis', HTTP_UNPROCESSABLE);
        ...
        try {
            ...
            // [AI:Claude] Vérifier si la colonne is_banned existe, sinon l'ajouter
            $stmt = $db->query("SHOW COLUMNS FROM users LIKE 'is_banned'");
            if ($stmt->rowCount() === 0) {
                $db->exec("ALTER TABLE users ADD COLUMN is_banned TINYINT(1) DEFAULT 0 AFTER role");
            }

            $stmt = $db->prepare("UPDATE users SET is_banned = ? WHERE id = ?");
            $stmt->execute([$isBanned ? 1 : 0, $userId]);

            $action = $isBanned ? 'banni' : 'débanni';
            error_log("[ADMIN BAN] {$userData['email']} a {$action} user {$userId} ({$user['email']})");

            Response::success([
                'is_banned' => $isBanned
            ], HTTP_OK, "Utilisateur {$action}");

        } catch (\Exception $e) {
            error_log('[AdminController] Erreur ban/unban : '.$e->getMessage());
            Response::serverError('Erreur lors du changement de statut');
        }
    }
