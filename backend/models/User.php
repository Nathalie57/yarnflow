...
class User extends BaseModel
{
     ...
     *
     * @param int $userId ID de l'utilisateur
     * @return bool True si l'utilisateur a accès aux tags
     */
    public function canUseTags(int $userId): bool
    {
        $user = $this->findById($userId);
        ...
        // [AI:Claude] Normaliser le type d'abonnement
        $normalizedType = $this->normalizeSubscriptionType($subscriptionType);

        $features = SUBSCRIPTION_FEATURES[$normalizedType] ?? SUBSCRIPTION_FEATURES['free'];

        return $features['can_use_tags'] ?? false;
    }

    /**
     * [AI:Claude] Vérifier si l'utilisateur peut filtrer par plusieurs tags (v0.15.0)
     *
     * @param int $userId ID de l'utilisateur
     * @return bool True si l'utilisateur peut utiliser le filtrage multi-tags
     */
    public function canFilterMultiTags(int $userId): bool
    {
        $user = $this->findById($userId);
        ...
        return $features['can_filter_multi_tags'] ?? false;
    }

    /**
     * [AI:Claude] Obtenir les fonctionnalités du plan de l'utilisateur (v0.15.0)
     *
     * @param int $userId ID de l'utilisateur
     * @return array Tableau des fonctionnalités
     */
    public function getSubscriptionFeatures(int $userId): array
    {
        ...
        return SUBSCRIPTION_FEATURES[$normalizedType] ?? SUBSCRIPTION_FEATURES['free'];
    }

    /**
     * [AI:Claude] Normaliser le type d'abonnement (gérer les variations)
     *
     * @param string $subscriptionType Type d'abonnement brut
