...
class AIPhotoService
{
    ...
    private function buildPrompt(string $type, string $context, string $itemName = '', string $modelGender = 'person', ?string $season = null): string
    {
        ...
        if (in_array($context, $babyGarmentWornContexts)) {
            ...
            return "Tu dois créer une nouvelle photo professionnelle {$contextDescription}{$seasonDescription}. Le vêtement doit être porté par un VRAI BÉBÉ HUMAIN (pas une poupée, pas un mannequin). ÉTAPES CRITIQUES : 1) Garde le vêtement fait main porté par le bébé. 2) RETIRE tous les éléments parasites : objets indésirables, fond original moche. 3) Place le bébé portant le vêtement dans le nouveau contexte avec une pose naturelle, confortable et sécurisante pour un bébé. RÈGLE ABSOLUE sur les détails visuels du vêtement porté : conserve EXACTEMENT les COULEURS, la TEXTURE, le MOTIF et tous les détails visuels. Conserve aussi EXACTEMENT le même NOMBRE d'éléments physiques discrets (boutons, fermetures, perles, attaches, pompons) que sur la photo originale : n'en ajoute aucun, n'en retire aucun. Tu PEUX changer l'angle de vue, la position du bébé dans l'espace pour créer une belle composition naturelle et douce, mais tu NE PEUX PAS changer l'apparence visuelle du vêtement lui-même (couleurs, motifs, texture). Le vêtement porté doit être bien mis en valeur dans une scène réaliste et attendrissante.";
        }
        ...
        if (in_array($context, $childGarmentWornContexts)) {
            ...
            return "Tu dois créer une nouvelle photo professionnelle {$contextDescription}{$seasonDescription}. Le vêtement doit être porté par {$childText} {$childAge} (pas une poupée, pas un mannequin). ÉTAPES CRITIQUES : 1) Garde le vêtement fait main porté par l'enfant. 2) RETIRE tous les éléments parasites : objets indésirables, fond original moche. 3) Place l'enfant portant le vêtement dans le nouveau contexte avec une pose naturelle, joyeuse et adaptée à son âge (debout, jouant, courant...). RÈGLE ABSOLUE sur les détails visuels du vêtement porté : conserve EXACTEMENT les COULEURS, la TEXTURE, le MOTIF et tous les détails visuels. Conserve aussi EXACTEMENT le même NOMBRE d'éléments physiques discrets (boutons, fermetures, perles, attaches, pompons) que sur la photo originale : n'en ajoute aucun, n'en retire aucun. Tu PEUX changer l'angle de vue, la position de l'enfant dans l'espace pour créer une belle composition naturelle et dynamique, mais tu NE PEUX PAS changer l'apparence visuelle du vêtement lui-même (couleurs, motifs, texture). Le vêtement porté doit être bien mis en valeur dans une scène réaliste et joyeuse.";
        }
        ...
        if (in_array($context, $childGarmentFlatContexts)) {
            return "Tu dois créer une nouvelle photo professionnelle {$contextDescription}{$seasonDescription}. ÉTAPES CRITIQUES : 1) ISOLE uniquement le vêtement enfant visible sur l'image originale. 2) RETIRE complètement tous les autres éléments : mains, bras, personnes, fond original, objets indésirables. 3) Place le vêtement enfant isolé complètement à plat sur la surface horizontale dans le nouveau contexte, comme s'il était naturellement posé par gravité, JAMAIS debout ou en position verticale. RÈGLE ABSOLUE sur les détails visuels : conserve EXACTEMENT les COULEURS, la TEXTURE, le MOTIF et tous les détails visuels de l'ouvrage. Conserve aussi EXACTEMENT le même NOMBRE d'éléments physiques discrets (boutons, fermetures, perles, attaches, pompons) que sur la photo originale : n'en ajoute aucun, n'en retire aucun. Tu PEUX changer l'angle de vue (vue du dessus, légèrement de côté, etc.) et la position sur la surface pour que ce soit naturel et bien composé, mais le vêtement doit toujours rester à plat horizontalement. Tu NE PEUX PAS changer l'apparence visuelle (couleurs, motifs, texture). Le vêtement enfant doit être seul et bien mis en scène.";
        }
        ...
        if ($isWornContext) {
            ...
            return "Tu dois créer une nouvelle photo professionnelle {$contextDescription}{$seasonDescription}. L'article doit être porté par {$modelText}, PAS un mannequin de vitrine en plastique. ÉTAPES CRITIQUES : 1) Garde l'ouvrage fait main porté par le modèle. 2) RETIRE tous les éléments parasites : mains qui tiennent artificiellement l'ouvrage (sauf si elles font naturellement partie de la pose), objets indésirables, fond original moche. 3) Place le modèle portant l'ouvrage dans le nouveau contexte avec une pose naturelle et appropriée. SI la photo originale est un selfie pris devant un miroir (téléphone visible dans le reflet, cadre du miroir visible, bras levé tenant le téléphone) : ignore complètement le téléphone, le miroir, son cadre et le bras qui tient le téléphone — recrée entièrement une nouvelle photo du modèle portant l'ouvrage dans une pose naturelle adaptée au nouveau contexte, sans aucune trace du miroir ni du téléphone. RÈGLE ABSOLUE sur les détails visuels de l'ouvrage porté : conserve EXACTEMENT les COULEURS, la TEXTURE, le MOTIF et tous les détails visuels. Conserve aussi EXACTEMENT le même NOMBRE d'éléments physiques discrets (boutons, fermetures, perles, attaches, pompons) que sur la photo originale : n'en ajoute aucun, n'en retire aucun. Tu PEUX changer l'angle de vue, la pose du modèle, la position dans l'espace pour créer une belle composition naturelle, mais tu NE PEUX PAS changer l'apparence visuelle de l'ouvrage lui-même (couleurs, motifs, texture). L'ouvrage porté doit être bien mis en valeur dans une scène réaliste.";
        }
        ...
        if ($isBabyContext || $isBabyGarmentFlatContext) {
            ...
            return "Tu dois créer une nouvelle photo professionnelle {$contextDescription}{$seasonDescription}. ÉTAPES CRITIQUES : 1) ISOLE uniquement le {$itemType} visible sur l'image originale. 2) RETIRE complètement tous les autres éléments : mains, bras, personnes, fond original, objets indésirables. 3) Place le {$itemType} isolé complètement à plat sur la surface horizontale dans le nouveau contexte, comme s'il était naturellement posé par gravité, JAMAIS debout ou en position verticale. RÈGLE ABSOLUE sur les détails visuels : conserve EXACTEMENT les COULEURS, la TEXTURE, le MOTIF et tous les détails visuels de l'ouvrage. Conserve aussi EXACTEMENT le même NOMBRE d'éléments physiques discrets (boutons, fermetures, perles, attaches, pompons) que sur la photo originale : n'en ajoute aucun, n'en retire aucun. Tu PEUX changer l'angle de vue (vue du dessus, légèrement de côté, etc.) et la position sur la surface pour que ce soit naturel et bien composé, mais le {$itemType} doit toujours rester à plat horizontalement. Tu NE PEUX PAS changer l'apparence visuelle (couleurs, motifs, texture). Le {$itemType} doit être seul et bien mis en scène.";
        }
        ...
        return "Tu dois créer une nouvelle photo professionnelle {$contextDescription}{$seasonDescription}. ÉTAPES CRITIQUES : 1) ISOLE uniquement l'ouvrage fait main visible sur l'image originale. 2) RETIRE complètement tous les autres éléments : mains, bras, personnes, fond original, objets indésirables. 3) Place l'ouvrage isolé dans le nouveau contexte avec une position, un angle et une pose NATURELS et APPROPRIÉS pour le contexte demandé. RÈGLE ABSOLUE sur les détails visuels de l'ouvrage : conserve EXACTEMENT les COULEURS, la TEXTURE, le MOTIF, la FORME et tous les détails visuels. Conserve aussi EXACTEMENT le même NOMBRE d'éléments physiques discrets (boutons, fermetures, perles, attaches, pompons) que sur la photo originale : n'en ajoute aucun, n'en retire aucun. Tu PEUX changer la position, l'angle de vue, l'orientation pour que ce soit naturel dans le nouveau contexte, mais tu NE PEUX PAS changer l'apparence visuelle de l'ouvrage (couleurs, motifs, texture). L'ouvrage doit être seul et bien mis en scène.";
    }
