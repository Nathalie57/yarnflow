...
const StashAllocationPanel = ({ projectId, onClose }) => {
  ...
  return (
    <div className="bg-white rounded-card shadow-xl w-full max-w-md max-h-[85vh] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div className="flex items-center gap-2.5">
          <FlowMascot pose="content" size={36} className="flex-shrink-0" />
          <div>
            <h2 className="font-semibold text-flow-ink">{t('ui.pickFromStash')}</h2>
            <p className="text-xs text-gray-500 mt-0.5">{t('ui.reserveBalls')}</p>
          </div>
        </div>
        <button onClick={onClose} className="p-1.5 rounded-control text-gray-400 hover:text-gray-600">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        {error && (
          <p className="text-sm text-red-600 bg-red-50 rounded-control px-4 py-3">{error}</p>
        )}
          ...
          <>
            ...
            {allocations.length > 0 && (
              <div className="space-y-2">
                {allocations.map(a => (
                  <div key={a.stash_entry_id} className="flex items-center gap-3 p-3 bg-primary-50 rounded-control border border-primary-100">
                    {a.photo_url ? (
                      <img src={API_URL + a.photo_url} alt="" className="w-10 h-10 rounded-control object-cover flex-shrink-0" />
                    ) : (
                      <div className="w-10 h-10 rounded-control flex-shrink-0 bg-primary-100" />
                    )}
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <button
                        ...
                        className="w-7 h-7 rounded-control border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-50 disabled:opacity-30 text-sm"
                      >−</button>
            ...
            {!adding && availableEntries.length > 0 && (
              <button
                ...
                className="w-full py-2.5 border-2 border-dashed border-gray-200 rounded-control text-sm text-gray-400 hover:border-primary-300 hover:text-primary-500 transition-colors flex items-center justify-center gap-2"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            ...
            {/* Formulaire d'ajout */}
            {adding && (
              <div className="border border-gray-200 rounded-control p-4 space-y-3">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {availableEntries.map(e => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-2.5 rounded-control border text-left transition-colors ${
                    ...
                    >
                      {e.photo_url ? (
                        <img src={API_URL + e.photo_url} alt="" className="w-9 h-9 rounded-control object-cover flex-shrink-0" />
                      ) : (
                        <div className="w-9 h-9 rounded-control flex-shrink-0" style={{ backgroundColor: e.color_hex || '#e5e7eb' }} />
                      )}
                      <div className="flex-1 min-w-0">
                ...
                {selectedEntry && (
                  <div className="flex items-center gap-3 pt-1">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-8 h-8 rounded-control border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-50">−</button>
                      <span className="w-8 text-center font-semibold text-gray-800">{quantity}</span>
                      <button onClick={() => setQuantity(q => Math.min(selectedEntry.quantity_available, q + 1))} className="w-8 h-8 rounded-control border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-50">+</button>
                    </div>
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    ...
                    className="flex-1 py-2 border border-gray-200 rounded-control text-sm text-gray-600 hover:bg-gray-50"
                  >{t('ui.cancel')}</button>
                  <button
                    onClick={handleAdd}
                    disabled={!selectedEntry || saving}
                    className="flex-1 py-2 bg-primary-600 text-white rounded-control text-sm font-semibold hover:bg-primary-700 disabled:opacity-50"
            ...
            {allocations.length === 0 && !adding && availableEntries.length === 0 && (
              <div className="text-center py-8 text-gray-400">
                <FlowMascot pose="onYVa" size={64} className="mx-auto mb-2" />
