...
const YarnStashStats = ({ stats }) => {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
      <div className="bg-white rounded-card border border-flow-mint p-4 text-center shadow-sm">
      ...
      <div className="bg-white rounded-card border border-flow-mint p-4 text-center shadow-sm">
        <div className="text-2xl font-bold text-primary-700">{formatWeight(stats.total_weight_g)}</div>
        <div className="text-xs text-gray-500 mt-1">{t('ui.ofWool')}</div>
      </div>

      <div className="bg-white rounded-card border border-flow-mint p-4 text-center shadow-sm">
        <div className="text-2xl font-bold text-primary-500">{formatYardage(stats.total_yardage_m)}</div>
